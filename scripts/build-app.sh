...
#  Location of the libs/micropython stub library built by Mynewt.  We will replace this stub by the MicroPython Library.
micropython_dir=$PWD/bin/targets/$mynewt_build_app/app/libs/micropython
micropython_dest=$micropython_dir/libs_micropython.a
...
#  If this is the very first build, do the Mynewt build to generate the rust_app, rust_libcore and micropython stubs.  This build will not link successfully but it's OK.
if [ ! -e $rust_app_dest ] || [ ! -e $micropython_dest ]; then
...
#  Copy MicroPython Library to libs_micropython.a, which is originally generated by libs/micropython.
echo ; echo "----- Copy MicroPython Library" 
#  TODO: Fix path
micropython_src=$PWD/../wasp-os/micropython/ports/mynewt/build-pinetime/micropython.a
set -x
cp $micropython_src $micropython_dest
set +x

#  Update the timestamp on libs_rust_libcore.a and libs_micropython.a so that Mynewt build won't overwrite the Rust libcore and MicroPython Library we have copied.
$ar_cmd s $rust_libcore_dest
$ar_cmd s $micropython_dest
