...
1. __Bootloader__: Located at the start of PineTime's Flash ROM, the open-source MCUBoot Bootloader is the first thing that PineTime runs upon booting. 
    ...
    If necessary, the Bootloader swaps the Active and Standby Firmware Images (for firmware update and rollback). Then it jumps to the Active Firmware code.
...
1. __User File System__: PineTime firmware may store the user's settings here. The contents of this flash area are preserved during firmware updates.
