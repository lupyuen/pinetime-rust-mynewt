...
# Advanced Topic: Distort the CHIP-8 Rendering
...
_How shall we get the Interpolation Points? Let's copy them from the diagram!_
...
- `(586, 105)` on the curved surface, normalised to `(-99, -84)`, maps to normalised `(-32, -16)` on CHIP-8

- `(618, 95)` on the curved surface, normalised to `(-65, -93)`, maps to normalised `(-21, -16)` on CHIP-8

- `(574, 137)` on the curved surface, normalised to `(-112, -56)`, maps to normalised `(-32, -11)` on CHIP-8
...
- Normalised `(0, 0)` on the curved surface maps to normalised `(1, 0)` on CHIP-8

- Normalised `(11, 0)` on the curved surface maps to normalised `(45, 0)` on CHIP-8

- Normalised `(0, 5)` on the curved surface maps to normalised `(1, 36)` on CHIP-8

We're now ready to interpolate the missing Physical and Virtual Pixels. We'll feed the above numbers to the [[spade] crate]([https://crates.io/crates/spade]) for interpolation, in two steps.

We'll feed the numbers for Virtual (X, Y) → Physical X first...
...
fn load_data() -> [Point3<f64>; 16] {
    [
        ...
        p( 1 as f64,  0 as f64,  0 as f64),  //  Virtual ( 1,  0) -> Physical X =  0
        p(45 as f64,  0 as f64, 11 as f64),  //  Virtual (45,  0) -> Physical X = 11
        p( 1 as f64, 36 as f64,  0 as f64),  //  Virtual ( 1, 36) -> Physical X =  0
        ...
```
_From https://github.com/lupyuen/interpolate-surface/blob/master/src/delaunay_creation.rs_

Then Virtual (X, Y) → Physical Y...
...
fn load_data() -> [Point3<f64>; 16] {
    [
        //  Generated by https://docs.google.com/spreadsheets/d/1G9kLS0Es6kwcMA3SC50w5-T-LBYi3NQeY98y7HOAovs/edit#gid=1875321785
        p( 1 as f64,  0 as f64, 0 as f64),  //  Virtual ( 1,  0) -> Physical X = 0
        p(45 as f64,  0 as f64, 0 as f64),  //  Virtual (45,  0) -> Physical X = 0
        p( 1 as f64, 36 as f64, 5 as f64),  //  Virtual ( 1, 36) -> Physical X = 5
        ...
```
_From https://github.com/lupyuen/interpolate-surface/blob/master/src/delaunay_creation.rs_
