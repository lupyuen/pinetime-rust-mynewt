...
static struct hal_spi_settings spi_temp_stm32_settings = {
...
STATS_SECT_START(temp_stm32_stat_section)
...
STATS_NAME_START(temp_stm32_stat_section)
    STATS_NAME(temp_stm32_stat_section, read_errors)
    STATS_NAME(temp_stm32_stat_section, write_errors)
    STATS_NAME(temp_stm32_stat_section, invalid_data_errors)
STATS_NAME_END(temp_stm32_stat_section)

/* Global variable used to hold stats data */
STATS_SECT_DECL(temp_stm32_stat_section) g_temp_stm32stats;

#define TEMP_STM32_LOG(lvl_, ...) \
    MODLOG_ ## lvl_(MYNEWT_VAL(TEMP_STM32_LOG_MODULE), __VA_ARGS__)
...
#if !MYNEWT_VAL(BUS_DRIVER_PRESENT)
    rc = hal_spi_config(sensor->s_itf.si_num, &spi_temp_stm32_settings);
    if (rc == EINVAL) {
...
#if MYNEWT_VAL(TEMP_STM32_SPEC_CALC)
/**
 ...
 */
static double
temp_stm32_compensate_temperature(int32_t rawtemp, struct temp_stm32_pdd *pdd)
{
    double var1, var2, comptemp;

    if (rawtemp == 0x80000) {
        TEMP_STM32_LOG(ERROR, "Invalid temp data\n");
        STATS_INC(g_temp_stm32stats, invalid_data_errors);
        return NAN;
    }
 ...
 */
static double
temp_stm32_compensate_pressure(struct sensor_itf *itf, int32_t rawpress,
                           struct temp_stm32_pdd *pdd)
{
    double var1, var2, p;
    int32_t temp;

    if (rawpress == 0x80000) {
        TEMP_STM32_LOG(ERROR, "Invalid press data\n");
        STATS_INC(g_temp_stm32stats, invalid_data_errors);
        return NAN;
    }

    if (!pdd->t_fine) {
        if(!temp_stm32_get_temperature(itf, &temp)) {
            (void)temp_stm32_compensate_temperature(temp, pdd);
        }
    }
 ...
 */
static double
temp_stm32_compensate_humidity(struct sensor_itf *itf, int32_t rawhumid,
                           struct temp_stm32_pdd *pdd)
{
    double h;
    int32_t temp;
    ...
    if (rawhumid == 0x8000) {
        TEMP_STM32_LOG(ERROR, "Invalid humidity data\n");
        STATS_INC(g_temp_stm32stats, invalid_data_errors);
        return NAN;
    }
                           ...
                           struct temp_stm32_pdd *pdd)
{
    ...
    if (rawpress == 0x80000) {
        TEMP_STM32_LOG(ERROR, "Invalid pressure data\n");
        STATS_INC(g_temp_stm32stats, invalid_data_errors);
        return NAN;
    }
 ...
 */
static float
temp_stm32_compensate_humidity(struct sensor_itf *itf, uint32_t rawhumid,
                           struct temp_stm32_pdd *pdd)
{
    int32_t h;
    int32_t temp;
 ...
 */
int
temp_stm32_is_calibrating(struct sensor_itf *itf, uint8_t *calibrating)
{
    uint8_t status;
    int rc;

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_STATUS, &status, 1);
    if (rc) {
        goto err;
    }

    *calibrating = (status & TEMP_STM32_REG_STATUS_IM_UP) != 0;
 ...
 */
static int
temp_stm32_get_calibinfo(struct sensor_itf *itf, struct temp_stm32_calib_data *bcd)
{
    int rc;
    uint8_t payload[33];
     ...
     * TEMP_STM32_REG_ADDR_DIG_T1, second one starting at
     * TEMP_STM32_REG_ADDR_DIG_H2.
     */

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_DIG_T1, payload, sizeof(payload));
    if (rc) {
        goto err;
    }
    ...
    memset(payload, 0, 7);
    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_DIG_H2, payload, 7);
    if (rc) {
        goto err;
    }
 ...
 * @param Sensor device TEMP_STM32 config
 *
 * @return 0 on success, non-zero on failure
 */
int
...
/**
 * Read multiple length data from TEMP_STM32 sensor over SPI
 *
 ...
 */
int
temp_stm32_readlen(struct sensor_itf *itf, uint8_t addr, uint8_t *payload,
               uint8_t len)
{
    int rc;
     ...
     */
    addr |= TEMP_STM32_SPI_READ_CMD_BIT;
    ...
    /* Send the address */
    retval = hal_spi_tx_val(itf->si_num, addr | TEMP_STM32_SPI_READ_CMD_BIT);
    if (retval == 0xFFFF) {
        rc = SYS_EINVAL;
        TEMP_STM32_LOG(ERROR, "SPI_%u register write failed addr:0x%02X\n",
                   itf->si_num, addr);
        STATS_INC(g_temp_stm32stats, read_errors);
        goto err;
    }
    ...
    for (i = 0; i < len; i++) {
        ...
        if (retval == 0xFFFF) {
            rc = SYS_EINVAL;
            TEMP_STM32_LOG(ERROR, "SPI_%u read failed addr:0x%02X\n",
                       itf->si_num, addr);
            STATS_INC(g_temp_stm32stats, read_errors);
            goto err;
        }
...
/**
 * Write multiple length data to TEMP_STM32 sensor over SPI
 *
 * @param register address
 * @param variable length payload
 ...
 */
int
temp_stm32_writelen(struct sensor_itf *itf, uint8_t addr, uint8_t *payload,
                uint8_t len)
{
    int rc;
    ...
    /* Send the address */
    rc = hal_spi_tx_val(itf->si_num, addr & ~TEMP_STM32_SPI_READ_CMD_BIT);
    if (rc == 0xFFFF) {
        rc = SYS_EINVAL;
        TEMP_STM32_LOG(ERROR, "SPI_%u register write failed addr:0x%02X\n",
                   itf->si_num, addr);
        STATS_INC(g_temp_stm32stats, write_errors);
        goto err;
    }
    ...
    for (i = 0; i < len; i++) {
        ...
        if (rc == 0xFFFF) {
            rc = SYS_EINVAL;
            TEMP_STM32_LOG(ERROR, "SPI_%u write failed addr:0x%02X:0x%02X\n",
                       itf->si_num, addr);
            STATS_INC(g_temp_stm32stats, write_errors);
            goto err;
        }
 ...
 */
int
temp_stm32_get_humidity(struct sensor_itf *itf, int32_t *humid)
{
    int rc;
    uint8_t tmp[2];

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_HUM, tmp, 2);
    if (rc) {
        goto err;
    }
    *humid = (tmp[0] << 8 | tmp[1]);
 ...
 */
int
temp_stm32_get_pressure(struct sensor_itf *itf, int32_t *press)
{
    int rc;
    uint8_t tmp[3];

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_PRESS, tmp, 3);
    if (rc) {
        goto err;
    }
...
/**
 * Resets the TEMP_STM32 chip
 *
 * @return 0 on success, non-zero on failure
 */
int
temp_stm32_reset(struct sensor_itf *itf)
{
    uint8_t txdata;

    txdata = 0xB6;

    return temp_stm32_writelen(itf, TEMP_STM32_REG_ADDR_RESET, &txdata, 1);
}
 ...
 */
int
temp_stm32_get_iir(struct sensor_itf *itf, uint8_t *iir)
{
    int rc;
    uint8_t tmp;

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_CONFIG, &tmp, 1);
    if (rc) {
        goto err;
    }

    *iir = ((tmp & TEMP_STM32_REG_CONFIG_FILTER) >> 5);
 ...
 */
int
temp_stm32_set_iir(struct sensor_itf *itf, uint8_t iir)
{
    int rc;
    uint8_t cfg;

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_CONFIG, &cfg, 1);
    if (rc) {
        goto err;
    }

    iir = cfg | ((iir << 5) & TEMP_STM32_REG_CONFIG_FILTER);

    rc = temp_stm32_writelen(itf, TEMP_STM32_REG_ADDR_CONFIG, &iir, 1);
    if (rc) {
        goto err;
    }
 ...
 */
int
temp_stm32_get_mode(struct sensor_itf *itf, uint8_t *mode)
{
    int rc;
    uint8_t tmp;

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_CTRL_MEAS, &tmp, 1);
    if (rc) {
        goto err;
    }

    *mode = (tmp & TEMP_STM32_REG_CTRL_MEAS_MODE);
 ...
 */
int
temp_stm32_set_mode(struct sensor_itf *itf, uint8_t mode)
{
    int rc;
    uint8_t cfg;

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_CTRL_MEAS, &cfg, 1);
    if (rc) {
        goto err;
    }

    cfg = cfg | (mode & TEMP_STM32_REG_CTRL_MEAS_MODE);

    rc = temp_stm32_writelen(itf, TEMP_STM32_REG_ADDR_CTRL_MEAS, &cfg, 1);
    if (rc) {
        goto err;
    }
 ...
 */
int
temp_stm32_get_oversample(struct sensor_itf *itf, sensor_type_t type,
                      uint8_t *oversample)
{
    int rc;
    uint8_t tmp;
    ...
    if (type & SENSOR_TYPE_AMBIENT_TEMPERATURE || type & SENSOR_TYPE_PRESSURE) {
        ...
        if (type & SENSOR_TYPE_AMBIENT_TEMPERATURE) {
            *oversample = ((tmp & TEMP_STM32_REG_CTRL_MEAS_TOVER) >> 5);
        }

        if (type & SENSOR_TYPE_PRESSURE) {
            *oversample = ((tmp & TEMP_STM32_REG_CTRL_MEAS_POVER) >> 2);
        }
    }

    if (type & SENSOR_TYPE_RELATIVE_HUMIDITY) {
        rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_CTRL_HUM, &tmp, 1);
        if (rc) {
            goto err;
        }
        *oversample = (tmp & TEMP_STM32_REG_CTRL_HUM_HOVER);
    }
 ...
 */
int
temp_stm32_set_oversample(struct sensor_itf *itf, sensor_type_t type,
                      uint8_t oversample)
{
    int rc;
    uint8_t cfg;
    ...
    if (type & SENSOR_TYPE_AMBIENT_TEMPERATURE || type & SENSOR_TYPE_PRESSURE) {
        ...
        if (type & SENSOR_TYPE_AMBIENT_TEMPERATURE) {
            cfg = cfg | ((oversample << 5) & TEMP_STM32_REG_CTRL_MEAS_TOVER);
        }

        if (type & SENSOR_TYPE_PRESSURE) {
            cfg = cfg | ((oversample << 2) & TEMP_STM32_REG_CTRL_MEAS_POVER);
        }
    ...
    if (type & SENSOR_TYPE_RELATIVE_HUMIDITY) {
        rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_CTRL_HUM, &cfg, 1);
        if (rc) {
            goto err;
        }

        cfg = cfg | (oversample & TEMP_STM32_REG_CTRL_HUM_HOVER);

        rc = temp_stm32_writelen(itf, TEMP_STM32_REG_ADDR_CTRL_HUM, &cfg, 1);
        if (rc) {
            goto err;
        }
    }
 ...
 */
int
temp_stm32_get_chipid(struct sensor_itf *itf, uint8_t *chipid)
{
    int rc;
    uint8_t tmp;

    rc = temp_stm32_readlen(itf, TEMP_STM32_REG_ADDR_CHIPID, &tmp, 1);
    if (rc) {
        goto err;
    }
 ...
 */
int
temp_stm32_set_sby_duration(struct sensor_itf *itf, uint8_t dur)
{
    int rc;
    uint8_t cfg;
    ...
    cfg = cfg | ((dur << 5) & TEMP_STM32_REG_CONFIG_STANDBY);

    rc = temp_stm32_writelen(itf, TEMP_STM32_REG_ADDR_CONFIG, &cfg, 1);
    if (rc) {
        goto err;
    }
 ...
 */
int
temp_stm32_get_sby_duration(struct sensor_itf *itf, uint8_t *dur)
{
    int rc;
    uint8_t tmp;
    ...
    *dur = tmp & TEMP_STM32_REG_CONFIG_STANDBY;
 ...
 */
int
temp_stm32_forced_mode_measurement(struct sensor_itf *itf)
{
    uint8_t status;
    int rc;
     ...
     */
    rc = temp_stm32_set_mode(itf, TEMP_STM32_MODE_FORCED);
    if (rc) {
        goto err;
    }
