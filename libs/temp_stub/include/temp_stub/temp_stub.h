...
//  Driver for stub Temperature Sensor.  On Blue Pill, the sensor is connected to port ADC1 on channel 16
//  This sensor is selected if TEMP_STub=1 in syscfg.yml.
...
#ifndef __TEMP_STUB_H__
#define __TEMP_STUB_H__
...
//  Configuration for the stub temperature sensor
struct temp_stub_cfg {
    sensor_type_t bc_s_mask;   //  Sensor data types that will be returned, i.e. temperature.
};

//  Device for the stub temperature sensor
struct temp_stub {
    struct os_dev dev;     //  Mynewt device
    struct sensor sensor;  //  Mynewt sensor
    struct temp_stub_cfg cfg;  //  Sensor configuration
    os_time_t last_read_time;   //  Last time the sensor was read.
};

/**
 * Create the stub temperature sensor instance.  Implemented in creator.c, function DEVICE_CREATE().
 */
void temp_stub_create(void);

/**
 * Return the default configuration for the stub temperature sensor.
 *
 * @param cfg  Pointer to the temp_stub_cfg device config
 *
...
/**
 * Initialize the stub temperature sensor.
 *
 * @param dev  Pointer to the temp_stub_dev device descriptor
 *
 * @return 0 on success, and non-zero error code on failure
 */
int temp_stub_init(struct os_dev *dev, void *arg);
 ...
 * @return 0 on success, and non-zero error code on failure
 */
int temp_stub_config(struct temp_stub *temp_stub, struct temp_stub_cfg *cfg);
