 ...
 * under the License.
 */
//  Adapted from repos/apache-mynewt-core/hw/drivers/adc/adc_stm32f4/src/adc_stm32f4.c
...
#define STM32L4_IS_DMA_ADC_CHANNEL(CHANNEL) ((CHANNEL) <= DMA_CHANNEL_2)

static DMA_HandleTypeDef *dma_handle[5];
static struct adc_dev *adc_dma[5];

struct stm32l4_adc_stats {
    uint16_t adc_events;
    uint16_t adc_error;
...
static struct stm32l4_adc_stats stm32l4_adc_stats;
...
static void
stm32l4_adc_dma_init(ADC_HandleTypeDef* hadc)
{
...
static void
stm32l4_adc_init(struct adc_dev *dev)
{
    struct stm32l4_adc_dev_cfg *adc_config;
...
static void
stm32l4_adc_uninit(struct adc_dev *dev)
{
 ...
 */
static int
stm32l4_adc_open(struct os_dev *odev, uint32_t wait, void *arg)
{
    DMA_HandleTypeDef *hdma;
    ADC_HandleTypeDef *hadc;
    struct stm32l4_adc_dev_cfg *cfg;
 ...
 */
static int
stm32l4_adc_close(struct os_dev *odev)
{
    struct adc_dev *dev;
 ...
 */
static int
stm32l4_adc_configure_channel(struct adc_dev *dev, uint8_t cnum,
        void *cfgdata)
{
    int rc;
    ADC_HandleTypeDef *hadc;
 ...
 */
static int
stm32l4_adc_set_buffer(struct adc_dev *dev, void *buf1, void *buf2,
        int buflen)
{
    struct stm32l4_adc_dev_cfg *cfg;
    int rc;
...
static int
stm32l4_adc_release_buffer(struct adc_dev *dev, void *buf, int buf_len)
{
    ADC_HandleTypeDef *hadc;
    struct stm32l4_adc_dev_cfg *cfg;
 ...
 */
static int
stm32l4_adc_sample(struct adc_dev *dev)
{
    int rc;
    ADC_HandleTypeDef *hadc;
    struct stm32l4_adc_dev_cfg *cfg;
...
static int
stm32l4_adc_read_buffer(struct adc_dev *dev, void *buf, int buf_len, int off,
                        int *result)
{
 ...
 */
static const struct adc_driver_funcs stm32l4_adc_funcs = {
        .af_configure_channel = stm32l4_adc_configure_channel,
        .af_sample = stm32l4_adc_sample,
        .af_read_channel = stm32l4_adc_read_channel,
        .af_set_buffer = stm32l4_adc_set_buffer,
        .af_release_buffer = stm32l4_adc_release_buffer,
        .af_read_buffer = stm32l4_adc_read_buffer,
        .af_size_buffer = stm32l4_adc_size_buffer,
};

/**
 * Callback to initialize an adc_dev structure from the os device
 * initialization callback.  This sets up a stm32l4_adc_device(), so
